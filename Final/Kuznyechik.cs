...
namespace Final
{
    class Kuznyechik
    {
        ...
        private byte[] _pi = _Pi;
        private byte[] _reversepi = _ReversePi;
        ...
        public byte[] Encrypt(byte[] data, byte[] key)
        {
            ...
            if (d.Length % 8 != 0)
            {
                string t = new string('!', 8 - d.Length % 8);
            ...
            {
                byte[] ba = Encoding.Unicode.GetBytes(d.Substring(0, 8));
                Console.WriteLine("   =====>    Шифрование    <=====   ");
                this.SetKey(key);
                byte[] block = new byte[BLOCK_SIZE];
                byte[] temp = new byte[BLOCK_SIZE];
                Array.Copy(ba, block, BLOCK_SIZE);
                for (int j = 0; j < 9; j++)
                {
                    LSX(ref temp, ref _subKeys[j], ref block);
        ...
        public byte[] Decrypt(byte[] data, byte[] key)
        {
            ...
            for (int i = 0; i < (len / 8); i++)
            {
                ...
                for (int j = 9; j > 0; j--)
                {
                    ReverseLSX(ref temp, ref _subKeys[j], ref block);
                    Array.Copy(temp, block, BLOCK_SIZE);
                }
